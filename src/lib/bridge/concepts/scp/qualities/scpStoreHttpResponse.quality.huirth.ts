...
>({
  ...
  reducer: (state, action) => {
    ...
    // C287 note (C) FLAG (UNTOUCHED · left as-is per W3): this is a direct in-place
    // Map mutation on `state.pendingHttpResponses` (a live-vs-snapshot Map seam). It is
    // the res-storage the unified quality path (C287 W1) now depends on — the strategy
    // path already relied on this same mutation working, so it is deliberately not
    // restructured. If a future stale-Map defect surfaces on close, START HERE.
