...
**Status**: shipped Suite Cascade System documentation · first published with `1.157.0` · maintained by the Meteoric Shipwright (the release steward) · the topic of an ongoing study.
...
| `Z` | **the hotfix count on that position** — a fix shipped without a completed cycle adds one; resets to `0` when `Y` advances | `0` |

So `0.950.2` reads: cycle 950, two hotfixes on it. `1.157.0` reads: cycle 1157, no hotfix yet. The count did not restart between them — it never stops.
...
| The Diametric SCP → the Renewable Resume | `0.950.2` → `1.157.0` | 207 | 0 | the running system re-assembles itself: the bridge survives its own restart and turn-over, two bridges coexist by name, every resume carries the current identity and the chosen model, the update reads the live instruction set |
