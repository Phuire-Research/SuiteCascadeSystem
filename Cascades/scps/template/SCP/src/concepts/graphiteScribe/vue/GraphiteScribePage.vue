...
import { GRAPHITESCRIBE_TABS, type GraphiteScribeDesignation, type GraphiteScribeTab } from '../graphiteScribe.type';
// Cycle 159 D1 · IUPA · graphiteScribe muxified per-page (no longer base)
...
const designations = ref<GraphiteScribeDesignation[]>([]);
const activeDesignationName = ref<string>('');
const activeTab = ref<GraphiteScribeTab>('info');
...
const activeDesignation = computed<GraphiteScribeDesignation | null>(() => {
...
function switchTab(tab: GraphiteScribeTab) {
  if (!muxium) return;
  muxium.dispatch(
    (muxium as Muxium<ClientMuxiumDeck>).deck.d.client.d.graphiteScribe.e.graphiteScribeSetActiveTab({ tab }),
  );
}
...
onMounted(() => {
  ...
  // Cycle 159 D1 · IUPA · landing supplies graphiteScribe as muxonomic page concept
  ...
  stagePlanner = muxium.plan<ClientMuxiumDeck>(
    'graphiteScribePageSubscription',
    ({ staging, stage, d__ }) =>
      staging(() => [
        stage(
          ({ d }) => {
            designations.value = d.client.d.graphiteScribe.k.designations.select();
            activeDesignationName.value = d.client.d.graphiteScribe.k.activeDesignationName.select();
            activeTab.value = d.client.d.graphiteScribe.k.activeTab.select();
            loadedDiamondContent.value = d.client.d.graphiteScribe.k.loadedDiamondContent.select();
            loadedOnyxContent.value = d.client.d.graphiteScribe.k.loadedOnyxContent.select();
            loadedBoundCascade.value = d.client.d.graphiteScribe.k.loadedBoundCascade.select();
            loadedFileSystemSheet.value = d.client.d.graphiteScribe.k.loadedFileSystemSheet.select();
          },
          {
            selectors: [
              d__.client.d.graphiteScribe.k.designations,
              d__.client.d.graphiteScribe.k.activeDesignationName,
              d__.client.d.graphiteScribe.k.activeTab,
              d__.client.d.graphiteScribe.k.loadedDiamondContent,
              d__.client.d.graphiteScribe.k.loadedOnyxContent,
              d__.client.d.graphiteScribe.k.loadedBoundCascade,
              d__.client.d.graphiteScribe.k.loadedFileSystemSheet,
            ],
          },
        ),
      ]),
...
<template>
  <div class="graphiteScribe-page">
    ...
    <nav class="tab-nav">
      <button
        v-for="tab in GRAPHITESCRIBE_TABS"
        :key="tab.value"
    ...
    <main class="tab-content">
      <section v-if="activeTab === 'info'" class="info-tab">
        ...
        <p v-else class="placeholder">
          (info-sheet not loaded — register sample designations from GraphiteScribeLanding for visual preview)
        </p>
      </section>
      ...
      <section v-else-if="activeTab === 'doviewer'" class="doviewer-tab">
        ...
        <div class="doviewer-grid">
          <div class="doviewer-pane">
            ...
            <p v-else class="placeholder">
              (Diamond not loaded — register sample designations from GraphiteScribeLanding)
            </p>
          </div>
          <div class="doviewer-pane">
            <h3>Onyx (Lambda)</h3>
            ...
            <p v-else class="placeholder">
              (Onyx not loaded — register sample designations from GraphiteScribeLanding)
            </p>
          </div>
        </div>
      </section>
