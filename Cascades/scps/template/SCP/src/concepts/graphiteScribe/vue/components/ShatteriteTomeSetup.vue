 ...
 *
 * Citation: EPOCH-DIAMOND-CODEEDITOR-SETUP-RESEARCH.md §2 Macro 1 SU
 ...
 * Citation: GraphiteScribeOnDemand.vue (triggerSendMessage await + controller computed)
 */
...
// page-bound session), NOT "any alive graphiteScribe match" — mirror CadmiumLanding.
const anchor = computed<ScsBridgeSessionEntry | undefined>(() =>
  sessionsList.value.find(
    (s) => s.graphiteScribeName === props.graphiteScribeName && s.isAnchor === true,
  ),
);
...
async function handleSubmit(): Promise<void> {
  ...
  console.log(
    '[ShatteriteTomeSetup STSC] handleSubmit · graphiteScribeName=', props.graphiteScribeName,
