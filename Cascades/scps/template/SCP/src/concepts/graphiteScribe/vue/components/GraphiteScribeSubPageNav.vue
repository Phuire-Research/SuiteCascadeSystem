...
/**
 * GraphiteScribe Sub-Page Navigation (Band A-6 HCD)
 *
 ...
 * click; the parent dispatches the Stratimux Quality (graphiteScribeSetActiveSubPage).
 *
 ...
 * Citation: graphiteScribe.subPageRegistry.ts (GRAPHITESCRIBE_SUB_PAGE_OPTIONS source).
 */
import type { GraphiteScribeSubPage } from '../../graphiteScribe.type';
import type { GraphiteScribeSubPageOption } from '../../graphiteScribe.subPageRegistry';

interface Props {
  options: GraphiteScribeSubPageOption[];
...
const emit = defineEmits<{
  (event: 'subPageSelected', value: GraphiteScribeSubPage): void;
}>();

function handleClick(option: GraphiteScribeSubPageOption) {
...
<template>
  <nav class="graphiteScribe-subpage-nav hifi-pane-base">
