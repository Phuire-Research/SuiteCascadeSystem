...
/**
 * GraphiteScribeCreateOverlay — the Create S8 STEP PIPELINE (C372 · the Creation Pipeline Overlay)
 *
 ...
 *   STEP 3 · THE PAGE — invokes graphiteScribe_page_create on the bridge via the controller's awaitable
 *                       triggerGraphiteScribePageCreate (the SAME /mcp tools/call idiom the GitM buttons ride,
 ...
 *
 * Citation: GraphiteScribeBiplane.vue requestTurnOverToB (:250 — the writeGitmTurnoverProgress + hard turn-over).
 * Citation: GitmSwordBButton.vue triggerGitmAction/triggerGitmMean (the /mcp tools/call idiom).
 * Citation: scsBridgeController triggerGraphiteScribePageCreate (the awaitable page-create result-parsing mean).
...
// The PascalCase concept name for graphiteScribe_page_create's `name` arg — the minted name with spaces
...
// ============================================================
// STEP 3 · THE PAGE (graphiteScribe_page_create · the awaitable controller mean · the ~30s window).
// ============================================================
...
async function runPageCreate() {
  ...
  try {
    ...
    const res = await ctrl.triggerGraphiteScribePageCreate({
