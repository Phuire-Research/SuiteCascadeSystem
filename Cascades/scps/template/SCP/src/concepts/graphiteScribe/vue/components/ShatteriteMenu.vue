 ...
 * `graphiteScribeName` as props — exactly like STSC receives its `sfsd`. The page muxium owns the
 ...
 *   the page's Anchor = the alive session whose `graphiteScribeName === props.graphiteScribeName`
 ...
 *   onMounted from GET /graphiteScribe-anchor-spawn/<graphiteScribeName>), the inert "Waiting…" stub is replaced
 *   by a Pewter-styled "Spawn + Anchor this domain" button. handleSpawnAnchor spawns via the
 *   EXISTING triggerSpawnGraphiteScribeSession (the bridge's claimAnchorIfUnclaimed auto-anchors the page
 *   anchor), then a readiness poll (mirror GraphiteScribeHomeLanding PPOL · 250ms/3000ms) focuses the new
 ...
 *   the per-callsite page spawn buttons (GraphiteScribeHomeLanding · PewterLanding), making the menu
...
// GET /graphiteScribe-anchor-spawn/<graphiteScribeName> (server resolves the per-Suite-8 Cascade.json ·
// DEFAULT 'prompt' on any failure · mirrors GraphiteScribeHomeLanding AD). 'prompt' = the menu surfaces a
...
// sidesteps the server anti-flood guard that a fresh spawn would trip). Mirrors GraphiteScribeHomeLanding /
...
// plan selector · mirrors GraphiteScribeHomeLanding PPOL-WUD).
...
// /graphiteScribe-anchor-spawn fetch can resolve BEFORE the offline prior hydrates → !anchor.value momentarily
// true → a spurious spawn that then steals the offline original via the fallback). This guard mirrors
// CadmiumLanding / GraphiteScribeHomeLanding PAOLR: settle over a window, then branch ONCE on the SETTLED
...
// component; no page wires its own floor). GET /graphiteScribe-menu/<graphiteScribeName> on mount → floorStage.
...
onMounted(() => {
  void fetch(s8MenuPath(props.graphiteScribeName))
...
const persistStageIndex = (idx: number): void => {
  void fetch(S8_MENU_STAGE_SET_PATH, {
    ...
    body: JSON.stringify({ designation: props.graphiteScribeName, currentStageIndex: idx }),
  }).catch(() => {
...
// /graphiteScribe-anchor-spawn/<graphiteScribeName> · DEFAULT 'prompt' on any failure · fire-and-forget · the
...
onMounted(() => {
  ...
  void fetch(s8AnchorSpawnPath(props.graphiteScribeName))
    ...
    .then((j: { anchorSpawn?: string }) => {
      ...
      // spawning over it (mirror CadmiumLanding / GraphiteScribeHomeLanding PAOLR three-branch).
...
// CadmiumLanding / GraphiteScribeHomeLanding PAOLR three-branch under a one-shot settle window: poll
...
function autoDecideAnchor(): void {
  ...
  autoDecidePoll = setInterval(() => {
    ...
    if (anchorAlive.value) {
      ...
      resolveOnce(() => {
        console.log('[ShatteriteMenu P1.1] AD-Auto · anchor ALIVE · no spawn · graphiteScribeName=', props.graphiteScribeName);
      });
      ...
      resolveOnce(() => {
        ...
        if (reengageTombstoned.value) {
          console.log('[ShatteriteMenu C472] anchor OFFLINE but TOMBSTONED · no auto re-engage · graphiteScribeName=', props.graphiteScribeName);
          return;
        }
        ...
        if (anchorSpawnMode.value !== 'auto') {
          console.log('[ShatteriteMenu C488] anchor OFFLINE · Auto-Spawn OFF · the button stands · graphiteScribeName=', props.graphiteScribeName);
          return;
        }
          ...
          void fetch(s8AnchorSpawnPath(props.graphiteScribeName))
            ...
            .then((j: { anchorSpawn?: string }) => {
              if (j.anchorSpawn === 'auto' && !spawning.value && !anchor.value) {
                console.log('[ShatteriteMenu P1.1] AD-Auto · NO anchor after settle · server-confirmed · spawn · graphiteScribeName=', props.graphiteScribeName);
                void handleSpawnAnchor();
              } else {
                console.log('[ShatteriteMenu C471] spawn double-check REFUSED · server mode=', j.anchorSpawn, '· graphiteScribeName=', props.graphiteScribeName);
              }
            })
            .catch(() => { /* unreachable server — never spawn blind */ });
        } else {
          console.log('[ShatteriteMenu C470] NO anchor after settle · auto-spawn OFF · the button stands · graphiteScribeName=', props.graphiteScribeName);
        }
      });
    }
  }, AD_STEP_MS);
...
const reengageTombstoneKey = (): string => `scsReengageTombstone:${props.graphiteScribeName}`;
...
watch(() => props.graphiteScribeName, (name, prev) => {
  if (name && !prev) {
...
watch([showSpawnOption, showReengageOption, anchorStatus], ([spawnOpt, reengOpt, status]) => {
  ...
  // (a cross-window set otherwise leaves this window's render stale until tab-in).
    ...
    '· spawning=', spawning.value, '· graphiteScribeName='.replace('graphiteScribeName','designation'), props.graphiteScribeName);
});
...
watch(settledAnchorStatus, (nowStatus, prevStatus) => {
  ...
  if (prevStatus === 'launched' && nowStatus === 'offline') {
    ...
    console.log('[ShatteriteMenu C472] anchor USER-CLOSED · tombstone SET · Auto-Spawn PRESERVED · graphiteScribeName=', props.graphiteScribeName);
  }
});

// SOE · spawn + anchor THIS domain from the menu (parallel to handleOption). Spawns via the EXISTING
// triggerSpawnGraphiteScribeSession (the bridge sets entry.graphiteScribeName BEFORE spawn → claimAnchorIfUnclaimed
// auto-anchors the page anchor), then a readiness poll (250ms/3000ms · mirror GraphiteScribeHomeLanding PPOL
// / CadmiumLanding PAOLRP) watches sessionsList for the launched anchor. On found → focus + clear.
// Fallback (a session for this graphiteScribeName appeared but did NOT auto-anchor): await triggerSetAnchor.
...
async function handleSpawnAnchor(): Promise<void> {
  ...
  ctrl.triggerSpawnS8Session(props.graphiteScribeName, origin || undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, anchorModelChoice.value ?? null);
  ...
  spawnPoll = setInterval(() => {
    void (async () => {
      ...
      // auto-stamp. If a same-graphiteScribeName anchor ALREADY exists (any status — e.g. an offline prior
      ...
      if (elapsedMs >= SOE_MAX_MS) {
        ...
        if (existingAnchor) {
          ...
          console.warn('[ShatteriteMenu SOE] spawn settled with no session for graphiteScribeName=', props.graphiteScribeName);
        }
        spawning.value = false;
      }
    })();
...
// poll / GraphiteScribeHomeLanding PPOL) watches sessionsList for the anchor flipping to 'launched'. On found
...
async function handleReengageAnchor(): Promise<void> {
  ...
  console.log('[ShatteriteMenu P1] re-engage offline anchor · graphiteScribeName=', props.graphiteScribeName, '· anchorId=', target.id);
  ...
  reengagePoll = setInterval(() => {
    ...
    if (elapsedMs >= REENGAGE_MAX_MS) {
      if (reengagePoll) { clearInterval(reengagePoll); reengagePoll = null; }
      console.warn('[ShatteriteMenu P1] re-engage did not settle alive for graphiteScribeName=', props.graphiteScribeName);
      isReengaging.value = false;
    }
  }, REENGAGE_STEP_MS);
}
...
// (1) GET the SORD-wrapped Skill/Strategy envelope from /graphiteScribe-skill-prime, now wrapped
...
async function primeSend(
  ...
  const url = `/graphiteScribe-skill-prime/${encodeURIComponent(props.graphiteScribeName)}?ref=${encodeURIComponent(ref)}&kind=${primeKind}`;
...
async function handleOption(option: MenuOption): Promise<void> {
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu SMSP] no alive Anchor for option · graphiteScribeName=', props.graphiteScribeName);
  ...
  try {
    if (option.kind === 'scs') {
      ...
      // server (/graphiteScribe-skill-prime), then relay the envelope to the live Anchor (triggerSendMessage)
...
async function handleSubmit(option: MenuOption, i: number): Promise<void> {
  ...
  if (typeof pairDirective === 'string' && pairDirective.length > 0) {
    ...
    const target = anchor.value;
    if (!ctrl || !target) {
      console.warn('[ShatteriteMenu TRP] no alive Anchor for pairing submit · graphiteScribeName=', props.graphiteScribeName);
      emit('option-selected', { label: option.label, kind: option.kind, ok: false });
      return;
    }
  ...
  const target = anchor.value;
  if (!ctrl || !target) {
    console.warn('[ShatteriteMenu MOIS] no alive Anchor for submit · graphiteScribeName=', props.graphiteScribeName);
    emit('option-selected', { label: option.label, kind: option.kind, ok: false });
    return;
  }
...
<template>
  ...
  >
    ...
    <div v-if="showSpawnOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">No Anchor yet for {{ graphiteScribeName }} — engage to begin.</span>
      <button
        type="button"
    ...
    <!-- P1 · ORPHAN-ANCHOR RE-ENGAGE · an anchor exists but is offline (status !== 'launched').
         ...
         render (no extra wiring). Mirrors GraphiteScribeHomeLanding / CadmiumLanding PPOL branch (2). -->
    <div v-else-if="showReengageOption" class="menu-stub menu-stub-spawn">
      <span class="menu-stub-text">
        Anchor for {{ graphiteScribeName }} is offline — re-engage to bring it back online.
      </span>
      <button
        type="button"
        class="menu-option-btn menu-spawn-btn"
