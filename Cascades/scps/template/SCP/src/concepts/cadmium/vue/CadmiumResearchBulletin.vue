...
/**
 * CadmiumResearchBulletin — BSBS · sidebar + detail render of the 3rd STCP output stream
 * (Diamond RAR · W5 → Diamond BSE · W3 restructure)
 *
 * The targeted-research article stream restructured from a flat card STACK into a two-channel
 * SIDEBAR + DETAIL layout:
 *   - SIDEBAR · a selectable list of compact META cards (title / preview / topic / sourceCount /
 *     filePath) sourced from the `researchBulletin: CadmiumArticle[]` prop (the live STCP store
 *     still drives this list reactively).
 *   - DETAIL · an overview pane that renders the SELECTED article's full markdownContent. The full
 *     body is fetched on-demand from the BDRP DETAIL channel (GET /cadmium-research-bulletin/:id)
 *     into a COMPONENT-SCOPED bodyCache Map — never re-fetched, never into Stratimux state (the
 *     prop is the light list; bodies stay local · mirrors ScsBridgeArchiveView bodyCache doctrine).
 *
 ...
 *
 * SERVER-ONLY parse stays server-side — this surface uses `fetch()` only (parseResearchBulletin
 * NEVER enters the client bundle). Preserves IAAF + CDBL/usePelb + `.markdown-pane` CSS.
 *
 * Pewter Tessera HiFi: Suite 2 Orange. Patterns: BSBS · BDRP · IAAF · CDBL/PELB · marked render
 * Citation: BSE-DIAMOND-WGB.md §W3 (BSBS sidebar+detail · bodyCache · selectCard · orphan guard)
 * Citation: ScsBridgeArchiveView.vue:52 (bodyCache reactive Map) · :82-92 (orphan guard)
 */
...
// BDRP · component-scoped detail bodyCache (mirrors ScsBridgeArchiveView.vue:52). Keys: articleId.
// Values: CadmiumArticle | 'loading' | 'error'. reactive(new Map()) so .set() re-renders with no
// counter refs; never re-fetches a present key; NEVER enters Stratimux state (bodies stay local).
...
// from a file path (may contain slashes). The parse lives server-side; this is a plain fetch.
function selectCard(articleId: string): void {
  ...
  void fetch('/cadmium-research-bulletin/' + encodeURIComponent(articleId))
...
// Orphan guard (mirrors ScsBridgeArchiveView.vue:82-92): if the live STCP relay full-replaces the
// list and the selected article is gone, clear the selection + its cache entry. Key on articleId.
watch(
  () => props.researchBulletin,
  (newList) => {
...
<template>
  <section class="bulletin-section">
    <div class="hifi-stamp">
      <h2 class="bulletin-section-title">Research Bulletin</h2>
      <span class="hifi-label">Targeted-Research Articles</span>
    </div>

    <div v-if="props.researchBulletin.length > 0" class="bulletin-split">
      <!-- SIDEBAR · selectable compact META cards (the live STCP store drives this list). -->
      <nav class="bulletin-sidebar">
        <button
          v-for="article in props.researchBulletin"
      ...
      <div class="bulletin-detail">
        <template v-if="selectedArticleId">
          ...
          <p v-else-if="bodyCache.get(selectedArticleId) === 'error'" class="detail-error">
            (failed to load this article · the targeted-research file may have changed)
          </p>
          <article
    ...
    <p v-else class="placeholder">
      (no targeted-research articles yet · a targeted Diamond writes them to
