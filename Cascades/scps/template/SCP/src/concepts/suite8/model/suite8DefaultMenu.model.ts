/**
 * Suite8 Default Menu Model File — the MINIMAL standing Shatterite Menu (PRE-EPOCH · decision 5)
 *
 * Pure model: zero Stratimux imports, zero dispatch. The DEFAULT MenuStage the Suite8HomeLanding
 * passes to ShatteriteMenu as its `defaultStage` prop. When NO live agent-authored menu.json stage
 * exists for this page's designation (shatteriteMenus[suite8Name] absent or EMPTY_MENU_STAGE), the
 ...
 *   - The spawn-anchor row is RENDERED AT THE PAGE LEVEL (Suite8HomeLanding · the AD `'prompt'`
 ...
 *     alive); the actual first-load spawn path is the page prompt-row → triggerSpawnSuite8Session.
 ...
 * Citation: suite8CreateScaffold.model.ts buildFoundingVermillionCommand (the founding shape).
...
const SUITE8_DEFAULT_FOUNDING_COMMAND = [
...
export const SUITE8_DEFAULT_MENU_STAGE: MenuStage = {
  stageIndex: 0,
  ...
  options: [
    {
      ...
      // affordance (triggerSpawnSuite8Session); this menu row is the standing explainer for it.
      ...
      label: 'Summon Anchor',
      kind: 'askMore',
      scsCommand: SUITE8_DEFAULT_FOUNDING_COMMAND,
