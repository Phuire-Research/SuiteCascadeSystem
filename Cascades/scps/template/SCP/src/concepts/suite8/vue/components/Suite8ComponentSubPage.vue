...
/**
 * Suite8 Component SubPage (CPLD · Band A-6 HCD)
 *
 * Component Plurality Live Doc. Docs + a LIVE example of what is usable once the
 * Suite8 concept is muxified — it plugs into the `suite8s` Record BY Key (Name).
 * The user selects any registered Suite8Entry Key (Name); the selection scopes the
 * PFGD pre-filtered session list (Suite8SessionList · A-5) to that Name, while the
 * ODSS spawn control (Suite8OnDemand · A-4) hosts the spawn + first-message unit.
 ...
 *
 * Wiring:
 *   - Suite8OnDemand (ODSS · A-4) = self-init island, NO props. Hosts spawn + send.
 *   - Suite8SessionList (PFGD · A-5) = self-init island, `suite8Name` prop. Receives
 ...
 * Citation: Suite8OnDemand.vue (ODSS · A-4) · Suite8SessionList.vue (PFGD · A-5).
 * Citation: MASTER-DIAMOND-SUITE8-CONCEPT-ASPIRANT.md §3 (CPLD) + §5 Diameter 3.
 */
import { ref, computed, watch } from 'vue';
import type { Suite8Entry } from '../../suite8.type';
import Suite8OnDemand from './Suite8OnDemand.vue';
import Suite8SessionList from './Suite8SessionList.vue';
...
const suite8Keys = computed<string[]>(() => Object.keys(props.suite8s));
// SB-DS6 · registered keys mapped to the ScsDropdown {value,label} shape (value === label === key).
const suite8KeyOptions = computed(() => suite8Keys.value.map((k) => ({ value: k, label: k })));
...
const selectedEntry = computed<Suite8Entry | null>(() =>
  selectedKey.value ? props.suite8s[selectedKey.value] ?? null : null,
);

// The selected entry's Name IS the PFGD filter predicate handed to the SessionList.
const selectedSuite8Name = computed<string>(() => selectedEntry.value?.name ?? '');
</script>

<template>
  <section class="suite8-component-subpage">
    <div class="cpld-doc hifi-pane-viridian">
      <h2 class="hifi-heading">Component · Plurality Live Doc (CPLD)</h2>
      <p class="cpld-description">
        The Suite8 concept plugs into the <strong>suite8s</strong> Record
      ...
      <div class="cpld-selector">
        ...
        <ScsDropdown
          v-model="selectedKey"
          :options="suite8KeyOptions"
          :placeholder="suite8Keys.length === 0 ? '(no Suite 8 registered)' : 'Select a Suite 8…'"
          :disabled="suite8Keys.length === 0"
    ...
    <div v-if="selectedEntry" class="cpld-live">
      ...
      <div class="cpld-live-block">
        ...
        <Suite8SessionList :suite8-name="selectedSuite8Name" />
      </div>
    </div>
