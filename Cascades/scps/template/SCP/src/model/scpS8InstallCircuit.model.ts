...
export const buildRequirementsMapperVermillion = (designation: string): string =>
  [
    ...
    '3. For each, read package.json: record the resolved version when present; a package IMPORTED but ABSENT from package.json is a HARD requirement finding.',
    '4. Judge inTemplateBaseline per package: the stock template SCP\'s dependencies (vue, stratimux, express, chokidar and the other package.json defaults every SCP ships) = true; anything the page added = false.',
    '',
...
export const buildInstallEntourageVermillion = (
  ...
  [
    ...
    '',
    'THE GATE (refuse-first):',
    `1. Read ${installRequirementsRelPath(designation)} in THIS (the source) SCP root. ABSENT anor installReady:false → write nothing, report the refusal in one line, and dissipate. The gate is the law.`,
    '',
