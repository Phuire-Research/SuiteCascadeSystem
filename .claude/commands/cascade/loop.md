...
Diamond Primer Protocol:
When the user provides any input with this command, that input IS the Diamond Primer — the initial scope declaration for a new Diamond engagement. The flow:
1. If `activeDiamond` is null in Cascade.json → EnterPlanMode → write the primer as a Banded Plan to `Cascades/Working/DIAMOND-TIER-1.md` → user approves → ExitPlanMode → set `activeDiamond` in Cascade.json
2. If `activeDiamond` already exists → the loop APPENDS to the existing Diamond. Unless the user explicitly directs otherwise, assume the Diamond is to be appended to — not replaced.
3. The `/loop` prompt generated by this command references the active Diamond. Each wake-up reads the Diamond and advances the next gate, appending results.
